...
가상환경을 활성화한 상태에서, `uvicorn`을 이용하여 API 서버를 구동합니다.
...
스캔된 일본어 신문 PDF 문서를 업로드 받아, 기사 단위 DOM 구조가 담긴 JSON 형태로 변환합니다.
...
#### 스키마 주요 노드 설명
1. **`BoundingBox` (`bbox`)**: 요소가 위치한 직사각형 영역 `(x0, y0, x1, y1)`. 신문 지면 상에서의 상대적 또는 절대적 좌표를 나타냅니다.
